...
fun main() {
    ...
    println(getLinkFromLinkedUrlOptimized("https://www.linkedin.com/posts/connectwell-industries-pvt-ltd_wherever-you-need-electrical-connections-activity-6971128396147798017-sjSf?utm_source=share&utm_medium=member_desktop"))
    println(getThumbnailOfLinkedInVideo("https://www.linkedin.com/posts/connectwell-industries-pvt-ltd_wherever-you-need-electrical-connections-activity-6971128396147798017-sjSf?utm_source=share&utm_medium=member_desktop"))
}
...
fun getThumbnailOfLinkedInVideo(linkedInUrl: String): String? {
    val doc =
        Jsoup.connect(linkedInUrl)
    val mainContent = doc.get().getElementById("main-content")
    val thumbUrl =
        mainContent?.firstElementChild()?.firstElementChild()?.select("video")?.first()?.attr("data-poster-url")
    return thumbUrl
}